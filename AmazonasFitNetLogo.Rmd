...
```{r calcPatchDistSimRCP85postDef, echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',eval=TRUE}
...
pexp %>% group_by(fct,theta,year) %>% filter( type == "pl", year>2000,decade!="Data") %>%  summarise(up_b=quantile(tot_patch,0.975),lo_b=quantile(tot_patch,0.025),tot_patch=median(tot_patch) ) %>%
      ...
      theme(legend.position = c(0.8, 0.88)) + geom_ribbon(aes(ymin=lo_b,ymax=up_b),alpha=0.2,linetype=0) + facet_wrap(~fct,nrow=2) + geom_point(data=pdata,color="black")
