...
# best2 <- msim %>% group_by( type,world_width,Initial_forest_density,forest_dispersal_distance, Forest_growth, siminputrow) %>%  summarise( pred_err_tot(cur_data(),mdat) ) %>% group_by(type,world_width)  %>% arrange(rmse) %>% slice_min(mabe,n=10) %>% mutate(fitted="totyear")
# 
# best <- bind_rows(best,best2)


saveRDS(best,file.path("Simulations","best_Amazon20yearsTot_lhs.rds"))

knitr::kable(best %>% arrange(mabe))
...
msim %>% inner_join(best  %>% filter(fitted=="maxyear", siminputrow==60)) %>% filter(world_width==449 ) %>% inner_join(mdat, by=c("Date" = "Date") ) %>%  
...
#
#msim %>% inner_join(best  %>% filter(fitted=="maxyear", siminputrow==60)) %>%  inner_join(mdat, by=c("Date" = "Date") ) %>%  mutate(year=year(Date)) %>%
msim %>% inner_join(best %>% filter(fitted=="maxyear")  %>% slice_min(mabe,n=1) ) %>% inner_join(mdat, by=c("Date" = "Date") ) %>%  mutate(year=year(Date)) %>%
  group_by(year,world_width) %>% summarise(burned_by_month=max(burned_by_month),total_patch=max(total_patch)) %>% ggplot(  aes(burned_by_month,total_patch, color=factor(world_width )))+ geom_point() + theme_bw() + xlab("Predicted") + ylab("Data") + 
...
* Need to REPEAT this part with the data Updated until 2021 (Climaterra data not yet updated to 2021!!!!)
...
# Simulations with best parameters theta = 90/110

## Simulations with patch size output for wordl_width = 450 -  theta 90 - theta 110

* Only with two best simulations used in the paper 

```{r sim_distinct449cluster90-110, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=TRUE}

best <- readRDS(file.path("Simulations","best_Amazon20yearsTot_lhs.rds"))

disp_best <- best %>% filter( world_width==449, fitted=="maxyear") %>% ungroup() %>% dplyr::select(forest_dispersal_distance) %>% deframe()
grow_best <- best %>% filter( world_width==449, fitted=="maxyear") %>% ungroup() %>% dplyr::select(Forest_growth) %>% deframe()
disp_best <- disp_best[c(5,6)]
grow_best <- grow_best[c(5,6)]

eval_times <- as.numeric(lapply(1:21, function(x){ymd("2000-12-31") + years(x) - ymd("1980-11-01") }))
ymd("1980-11-01") + days(eval_times)

nl@experiment <- experiment(expname="Amazon20years449spNewTheta90-110",
                            outpath=outpath,
                            repetition=1,
                            tickmetrics="true",
                            idsetup="setup",
...
## Calc exponents best fitted wordl_width = 450 -  theta 90 and 110

```{r calcPatchDist449_spNewTheta90-110, echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',eval=TRUE}
...
#
sim <- read_netlogo_simul(file.path(outpath,"Amazon20years449spNewTheta90-110_distinct.csv"),skip=0)
...
saveRDS(pexp, file.path("Simulations", "simExponentsAmazonBestTheta90-110.rds"))
```


# Simulations with the fitted bF for the data period - theta 90

* With "use-fire-prob-se" = false Amazon20years449fittedBFnose_simple.csv

```{r sim_simple449fitted_gamBF, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
...
* Amazon20years449fittedBFnose_simple.csv   use-fire-prob-se = false
* Amazon20years449fittedBF_simple.csv       use-fire-prob-se = true

```{r calcPatchDist449_gamBF, echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',eval=FALSE}
...
# Simulations with the fitted bF for the data period - theta 110
...
```{r sim_simple449clusterRCP45nose, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
...
nl@experiment <- experiment(expname="Amazon39years449rcp45nose",
                            outpath=outpath,
                            repetition=1,
                            tickmetrics="true",
                            idsetup="setup",
...
#
list.files(outpath)
sim <- read_netlogo_simul(file.path(outpath,"Amazon39years449rcp45nose_simple.csv"),skip=0)
...
saveRDS(pexp, file.path("Simulations", "simExponentsAmazonRCP45nose.rds"))
```
...
```{r sim_simple449clusterRCP85nose, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=FALSE}

eval_times <- as.numeric(lapply(0:39, function(x){ymd("2020-12-31") + years(x) - ymd("1980-11-01") }))
ymd("1980-11-01") + days(eval_times)

nl@experiment <- experiment(expname="Amazon39years449rcp85nose",
                            outpath=outpath,
                            repetition=1,
                            tickmetrics="true",
                            idsetup="setup",
...
#
list.files(outpath)
sim <- read_netlogo_simul(file.path(outpath,"Amazon39years449rcp85nose_simple.csv"),skip=0)
# Calculate power law exponents and max/total patch size by year 
#
pexp <- tibble()
...
saveRDS(pexp, file.path("Simulations", "simExponentsAmazonRCP85nose.rds"))
```
...
pexp <- pexp %>% bind_rows(pexp0,pexp1,patch_sizes_p) %>% mutate(tot_patch= 100 * tot_patch, max_patch= max_patch*100, num_patch=100* num_patch)  ## Make %
...
p4 <- pexp %>% group_by(decade,random_seed,Date) %>% filter(AICc == min(AICc), type == "pl",exp_p_value<0.05) %>% bind_rows(patch_dfp) %>% ggplot( aes(fct_relevel(decade,c("Data","Simul Data","Simul GAM 90")),expo,fill=decade)) + geom_violin() + scale_fill_viridis_d(guide=FALSE) +    scale_color_viridis_d() + ylab("Power Law Exponent")  +  stat_summary(fun.y=median, geom="point", size=2) + xlab("") +
     scale_x_discrete(labels=c("Data", "Simul Data", TeX("Simul GAM $\\theta=90$"),TeX("Simul GAM $\\theta=110$")))
...
pexp %>% group_by(decade,year) %>% filter( type == "pl", year>2000) %>%  
      ggplot(aes(year,tot_patch,color=decade)) + geom_jitter(alpha=0.8) + scale_color_viridis_d(name="") + xlab("Year") + ylab("Total Fire size%") +
      theme(legend.position = c(0.8, 0.88)) 

pexp %>% group_by(decade,year) %>% filter( type == "pl", year>2000) %>%  summarise(up_b=quantile(tot_patch,0.95),lo_b=quantile(tot_patch,0.05),tot_patch=median(tot_patch) ) %>%
      ggplot(aes(year,tot_patch,color=decade)) + geom_line(alpha=0.8) + scale_color_viridis_d(name="") + xlab("Year") + ylab("Total Fire size%") +
      theme(legend.position = c(0.8, 0.88)) 
ggsave("figure/Amazon_TotalFire_year_SimulDataGam.png",width=8,height=6,units="in",dpi=600)

pexp %>% group_by(decade,year) %>% filter( type == "pl", year>2000) %>%  summarise(max_patch=median(max_patch))%>%
      ggplot(aes(year,max_patch,color=decade)) + geom_point(alpha=0.8) + scale_color_viridis_d(name="") +       xlab("Year") + ylab("Max Fire size%") +
      theme(legend.position = c(0.8, 0.88)) 
...
#
pexp <- readRDS(file.path("Simulations", "simExponentsAmazonfittedBFnose.rds")) %>%   filter(world_width==449) %>% mutate(decade="2001-2020",tot_patch= tot_patch/(450*450),max_patch=max_patch/(450*450),num_patch=num_patch/(450*450), year= year(Date),theta=90)
...
# Max size
#
pexp %>% group_by(random_seed,Date) %>% filter( type == "pl") %>% mutate(decade=factor(decade)) %>% ggplot(aes(fct_relevel(decade,c("Data","2001-2020")),max_patch,fill=decade)) + geom_violin() + scale_fill_viridis_d(guide=FALSE)  + ggtitle("Amazon RCP8.5") + ylab("Max Fire Size (%)")  +  stat_summary(fun.y=median, geom="point", size=2) + xlab("") + facet_wrap( ~theta,drop=TRUE,scales="free_x")
ggsave("figure/Amazon_Max_Size_theta90-110_RCP8.5.png",width=8,height=6,units="in",dpi=600)

# Total size
#
pexp %>% group_by(random_seed,Date) %>% filter( type == "pl") %>% mutate(decade=factor(decade)) %>%
  ggplot(aes(fct_relevel(decade,c("Data","2001-2020")),tot_patch,fill=decade)) + geom_violin() + scale_fill_viridis_d(guide=FALSE) + scale_color_viridis_d() +
  ggtitle("Amazon RCP8.5") + ylab("Total Fire Size (%)")  +  stat_summary(fun=median, geom="point", size=2) + xlab("") + facet_wrap( ~theta,,scales="free_x")
ggsave("figure/Amazon_Tot_Size_theta90-110_RCP8.5.png",width=8,height=6,units="in",dpi=600)

#
# Number of Fires 
#
pexp %>% group_by(random_seed,Date) %>% filter( type == "pl") %>% mutate(decade=factor(decade)) %>% ggplot(aes(fct_relevel(decade,c("Data","2001-2020")),num_patch,fill=decade)) + geom_violin() + scale_fill_viridis_d(guide=FALSE) +    scale_color_viridis_d() + ggtitle("Amazon RCP8.5") + ylab("Number of Fires (%)")  +  stat_summary(fun=median, geom="point", size=2) + xlab("") + facet_wrap( ~theta,drop=TRUE,scales="free_x")
...
pexp <- bind_rows(pexp,readRDS(file.path("Simulations", "simExponentsAmazonRCP85theta110.rds"))  %>%  filter(world_width==449) %>%
  mutate(tot_patch= tot_patch/(450*450),max_patch=max_patch/(450*450),num_patch=num_patch/(450*450),year=year(Date),
         decade=trunc((year - 2000 )/10)*10+2000,decade=paste0(decade , "-", decade+9),decade=factor(decade),theta=110,RCP="8.5"))
...
# Total size
#
pexp %>% group_by(random_seed,Date) %>% filter( type == "pl") %>% mutate(decade=factor(decade),decade=fct_relevel(decade,c("Data","2001-2020"))) %>% ggplot(aes(decade,tot_patch,fill=decade)) + geom_violin() + scale_fill_viridis_d(guide=FALSE) +    scale_color_viridis_d() + ylab("Total Fire Size (%)")  +  stat_summary(fun=median, geom="point", size=2) + xlab("") + facet_grid( RCP ~theta,drop=TRUE,scales="free_x")
ggsave("figure/Amazon_Tot_Size_theta90-110_RCP4.5-8.5.png",width=8,height=6,units="in",dpi=600)

# Max size
#
pexp %>% group_by(random_seed,Date) %>% filter( type == "pl") %>% mutate(decade=factor(decade),decade=fct_relevel(decade,c("Data","2001-2020"))) %>% ggplot(aes(decade,max_patch,fill=decade)) + geom_violin() + scale_fill_viridis_d(guide=FALSE)  +  ylab("Max Fire Size (%)")  +  stat_summary(fun.y=median, geom="point", size=2) + xlab("") + facet_grid( RCP ~theta,drop=TRUE,scales="free_x")
