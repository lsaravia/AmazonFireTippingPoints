...
```{r gen_fit_lhs449exp2, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=TRUE}

# A new experiment with a bigger lattice to check size effects
#
nl@experiment <- experiment(expname="Amazon20years450exp02",
                            outpath=outpath,
                            repetition=1,
                            tickmetrics="true",
                            idsetup="setup",
...
#best <- msim0 %>% group_by( type,world_width,forest_dispersal_distance, Forest_growth, siminputrow) %>%  summarise( pred_err_max(cur_data(),mdat) ) %>% group_by(type,world_width)  %>% arrange(rmse) %>% slice_min(mabe,n=10)
...
```{r comp_fit_lhs449cluster, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
