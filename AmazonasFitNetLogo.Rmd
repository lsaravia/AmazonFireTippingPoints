...
```{r gen_fit_lhs449exp, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
...
* I repeated these simulations with type fixed and periodic for comparison but here only the periodic forced with data is included.
...
pexp <- pexp %>% filter(!(world_width==449 & sim_type=="periodic") )   ## -----> recalculate to get the number of patches
...
```{r calcPatchDistSimRCP45, echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',eval=TRUE}
...
pexp <- readRDS(file.path("Simulations", "simExponentsAmazonfittedBF.rds")) %>%   filter(world_width==449,forest_dispersal_distance==2.81) %>% mutate(decade="Simul GAM",tot_patch= tot_patch/(450*450),max_patch=max_patch/(450*450),num_patch=num_patch/(450*450), year= year(Date))

#
# Read simulations based on actual data
#
pexp1 <- readRDS(file.path("Simulations", "fittedExponentsAmazon20years.rds")) %>%
  filter(world_width==449,forest_dispersal_distance==2.81) %>% mutate(decade="Simul Data",tot_patch= tot_patch/(450*450),max_patch=max_patch/(450*450), num_patch=num_patch/(450*450), year= year(Date))
pexp <- pexp %>% bind_rows(pexp1,patch_sizes_p)
...
# Read simulations based on actual data
#
pexp_data <- readRDS(file.path("Simulations", "simExponentsAmazonfittedBF.rds")) %>%   filter(world_width==449,forest_dispersal_distance==2.81) %>% mutate(decade="2010-2019",tot_patch= tot_patch/(450*450),max_patch=max_patch/(450*450),num_patch=num_patch/(450*450), year= year(Date))
