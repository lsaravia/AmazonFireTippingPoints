...
#
pexp <- readRDS(file.path("Simulations", "simExponentsAmazonfittedBF.rds")) %>%   filter(world_width==449) %>% mutate(decade="2001-2020",tot_patch= tot_patch/(450*450),max_patch=max_patch/(450*450),num_patch=num_patch/(450*450), year= year(Date),theta=61,RCP="4.5")
...
# 
pexp <- bind_rows(pexp,readRDS(file.path("Simulations", "simExponentsAmazonRCP85_59post.rds")) %>% filter(world_width==449) %>% 
...
pexp1 <- readRDS(file.path("Simulations", "simExponentsAmazonRCP45_59post.rds")) %>% filter(world_width==449) %>% 
                     mutate(tot_patch= tot_patch/(450*450),max_patch=max_patch/(450*450),,num_patch=num_patch/(450*450),
                    year=year(Date),decade=trunc((year - 2000 )/10)*10+2000,decade=paste0(decade , "-", decade+9),decade=factor(decade),theta=61)
...
# Add Data
#
pexp <- pexp %>% bind_rows(patch_sizes_p %>% mutate(theta=61,RCP="4.5")) %>%  mutate(tot_patch= 100 * tot_patch, max_patch= max_patch*100, num_patch=100* num_patch)## Make %
pexp %>% count(decade,theta)
