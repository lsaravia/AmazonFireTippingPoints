...
```{r sim_simple449postRCP45_All_59_2, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=TRUE}
...
grow_best <- best %>% ungroup() %>% dplyr::select(Forest_growth) %>% deframe()
ini_best  <- best %>% ungroup() %>% dplyr::select(Initial_forest_density) %>% deframe()
disp_best <- disp_best[1:60]
grow_best <- grow_best[1:60]
ini_best <- ini_best[1:60]
...
#
#
nl@experiment <- experiment(expname="Amazon59years449PostRcp45_all1",
                            outpath=outpath,
                            repetition=1,
                            tickmetrics="true",
                            idsetup="setup",
