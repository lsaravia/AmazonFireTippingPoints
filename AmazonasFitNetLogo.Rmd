...
```{r sim_simple449postRCP45_deforest59_2, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=TRUE}
...
sim <- read_netlogo_simul(file.path(outpath,"Amazon59years449PostRcp45_def1_distinct.csv"),skip=0)
...
saveRDS(pexp, file.path("Simulations", "simExponentsAmazonRCP45_59postDef.rds"))
```
...
p1 <- pexp  %>% group_by(random_seed,Date) %>% filter( type == "pl") %>% mutate(theta=factor(if_else(decade=="Data","Data",as.character(theta))), decade=if_else(decade=="Data", "2001-2021",decade))
