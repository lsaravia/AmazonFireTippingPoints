...
pexp1 <- readRDS(file.path("Simulations", "simExponentsAmazonRCP45_59postDef.rds")) %>% filter(world_width==449) %>% 
                     mutate(tot_patch= tot_patch/(450*450),max_patch=max_patch/(450*450),,num_patch=num_patch/(450*450),
                    year=year(Date),decade=trunc((year - 2000 )/10)*10+2000,decade=paste0(decade , "-", decade+9),decade=factor(decade),theta=61,RCP="4.5 + Def")
...
pexp <- pexp %>% bind_rows(patch_sizes_p %>% mutate(theta=61,RCP="4.5"), 
                           patch_sizes_p %>% mutate(theta=61,RCP="4.5 + Def"),
                           patch_sizes_p %>% mutate(theta=61,RCP="8.5")) %>%  mutate(tot_patch= 100 * tot_patch, max_patch= max_patch*100, num_patch=100* num_patch) %>% filter(year>2000)## Make %
pexp %>% count(decade,theta)
...
pdata <- pdata %>% bind_rows(patch_sizes_p %>% mutate(theta=61,RCP="4.5 + Def") %>%  mutate(tot_patch= 100 * tot_patch, max_patch= max_patch*100, num_patch=100* num_patch)) %>% filter(year>2000)
...
#
p1 <- pexp  %>% group_by(random_seed,Date) %>% filter( type == "pl") %>% mutate(theta=factor(if_else(decade=="Data","Data",as.character(theta))), decade=if_else(decade=="Data", "2001-2021",decade)) %>% filter( RCP != "4.5 + Def")
...
#
p1 <- pexp  %>% group_by(random_seed,Date) %>% filter( type == "pl") %>% mutate(theta=factor(if_else(decade=="Data","Data",as.character(theta))), decade=if_else(decade=="Data", "2001-2021",decade)) %>% filter(RCP == "4.5" | RCP == "4.5 + Def")
...
#
p1 <- pexp  %>% group_by(random_seed,Date) %>% filter( type == "pl") %>% mutate(theta=factor(if_else(decade=="Data","Data",as.character(theta))), decade=if_else(decade=="Data", "2001-2021",decade)) %>% filter(RCP == "8.5" | RCP == "8.5 + Def")
...
#
pexp %>% group_by(random_seed,Date) %>% filter(AICc == min(AICc), type == "pl") %>% bind_rows(patch_dfp %>% mutate(theta=61,RCP="4.5")) %>% 
