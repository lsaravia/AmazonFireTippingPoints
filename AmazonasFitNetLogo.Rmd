...
} else if(Sys.info()['nodename'] =="Mac-mini.local") {
  data_path <- "~/GitProjects/AmazonTippingPoint/Data"  
...
} else if(Sys.info()['nodename'] =="Mac-mini.local") {                     # Mac Pro
  simfolder <- "/Users/leonardosaravia/GitProjects/fireNL"  
  netlogopath <- file.path("/Applications/NetLogo\ 6.4.0")                   
  Sys.setenv(JAVA_HOME = "/Library/Java/JavaVirtualMachines/liberica-jdk-17.jdk/Contents/Home")
...
# Previous simulations Amazon59years449PostRcp45_60 & Amazon59years449PostRcp45
#
...
```{r calcPatchDistSimRCP85post, echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',eval=FALSE}
...
# Simulations with best parameters and patch size distribution RCP 4.5 (2021 - 2059) with Deforestation 

* wordl_width = 450 
* RCP 4.5
* Have to run the first 40 [1:40] and then the last 60 [41:100] so I generated 2 files

Amazon59years449PostRcp45_def1 = [1:60]
Amazon59years449PostRcp45_def2 = [61:100]


```{r sim_simple449postRCP45_deforest59_2, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
...
grow_best <- best %>% ungroup() %>% dplyr::select(Forest_growth) %>% deframe()
ini_best  <- best %>% ungroup() %>% dplyr::select(Initial_forest_density) %>% deframe()
disp_best <- disp_best[61:100]
grow_best <- grow_best[61:100]
ini_best <- ini_best[61:100]
# eval_times <- as.numeric(lapply(0:9, function(x){ymd("2011-12-31") + years(x) - ymd("1980-11-01") }))
# ymd("1980-11-01") + days(eval_times)
...
# Previous simulations Amazon59years449PostRcp45_60 & Amazon59years449PostRcp45
#
nl@experiment <- experiment(expname="Amazon59years449PostRcp45_def2",
                            outpath=outpath,
                            repetition=1,
                            tickmetrics="true",
                            idsetup="setup",
                            ...
                            constants = list("world-width" = 449,
                                             "world-height" = 449,
                                               "fire-prob-filename"="\"Data/Predicted_bF_rcp45_def.csv\"",
                                               "Save-view" =  "false",
                                               "video" = "false",
                                               "end-simulation"= 28918,
                                               "Periodicity" = "false",
...
# Calculate patch distributions from best fitted parameteres forced with RCP 4.5 and deforestation

```{r calcPatchDistSimRCP45postDef, echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',eval=FALSE}

# Read previous simulations for RCP 4.5 and fitted parameters
#
list.files(outpath,"Amazon59years449PostRcp45.*\\.csv")

sim <- read_netlogo_simul(file.path(outpath,"Amazon59years449PostRcp45_def2_distinct.csv"),skip=0)
#sim <- bind_rows(sim,read_netlogo_simul(file.path(outpath,"Amazon59years449PostRcp45_def2_distinct.csv"),skip=0))
...
pexp <- bind_rows(p_df)
plan(sequential)
pexp %>% filter(type=="pl") %>% mutate(tot_patch= tot_patch/(450*450)*100)  %>% 
         summarise(up_b=quantile(tot_patch,0.975),lo_b=quantile(tot_patch,0.025),tot_patch=median(tot_patch))

# Load previous simulations and bind the total file
#
pexp0 <- readRDS(file.path("Simulations", "simExponentsAmazonRCP45_59postDef.rds")) 
saveRDS(bind_rows(pexp0,pexp), file.path("Simulations", "simExponentsAmazonRCP45_59postDef.rds"))
```


# Simulations with best parameters and patch size distribution RCP 8.5 (2021 - 2059) with Deforestation 

* world_width = 450 
* RCP 8.5
* Have to run the first 40 [1:60] and then the last 40 [61:100] so I generated 2 files

Amazon59years449PostRcp85_def1 = [1:60]
Amazon59years449PostRcp85_def2 = [61:100]


```{r sim_simple449postRCP85_deforest59_2, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
...
# Previous simulations Amazon59years449PostRcp45_60 & Amazon59years449PostRcp45
#
nl@experiment <- experiment(expname="Amazon59years449PostRcp85_def2",
                            outpath=outpath,
                            repetition=1,
                            tickmetrics="true",
                            idsetup="setup",
                            ...
                            constants = list("world-width" = 449,
                                             "world-height" = 449,
                                               "fire-prob-filename"="\"Data/Predicted_bF_rcp85_def.csv\"",
                                               "Save-view" =  "false",
                                               "video" = "false",
                                               "end-simulation"= 28918,
                                               "Periodicity" = "false",
...
# Calculate patch distributions from best fitted parameteres forced with RCP 8.5 and deforestation

```{r calcPatchDistSimRCP85postDef, echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',eval=FALSE}

# Read previous simulations for RCP 8.5 and fitted parameters
#
list.files(outpath,"Amazon59years449PostRcp85.*\\.csv")

sim <- read_netlogo_simul(file.path(outpath,"Amazon59years449PostRcp85_def2_distinct.csv"),skip=0)
...
#saveRDS(pexp, file.path("Simulations", "simExponentsAmazonRCP85_59postDef.rds"))
# Load previous simulations and bind the total file
#
pexp0 <- readRDS(file.path("Simulations", "simExponentsAmazonRCP85_59postDef.rds")) 
saveRDS(bind_rows(pexp0,pexp), file.path("Simulations", "simExponentsAmazonRCP85_59postDef.rds"))
```


# Simulations with best parameters and patch size distribution RCP 4.5 (2021 - 2059) all GCM (NO Climaterra) 

* wordl_width = 450 
* RCP 4.5
* Have to run the first 40 [1:40] and then the last 60 [41:100] so I generated 2 files

Amazon59years449PostRcp45_all1 = [1:60]
Amazon59years449PostRcp45_all2 = [61:100]


```{r sim_simple449postRCP45_All2_59, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=TRUE}
...
#
#
nl@experiment <- experiment(expname="Amazon59years449PostRcp45_all2",
                            outpath=outpath,
                            repetition=1,
                            tickmetrics="true",
                            idsetup="setup",
                            ...
                            constants = list("world-width" = 449,
                                             "world-height" = 449,
                                               "fire-prob-filename"="\"Data/Predicted_bF_all_rcp45.csv\"",
                                               "Save-view" =  "false",
                                               "video" = "false",
                                               "end-simulation"= 28918,
                                               "Periodicity" = "false",
...
# Calculate patch distributions from best fitted parameteres forced with RCP 4.5 all GCM (NO Climaterra)

```{r calcPatchDistSimRCP45postAll, echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',eval=TRUE}

# Read previous simulations for RCP 4.5 and fitted parameters
#
list.files(outpath,"Amazon59years449PostRcp45.*\\.csv")

#sim <- read_netlogo_simul(file.path(outpath,"Amazon59years449PostRcp45_all1_distinct.csv"),skip=0)
sim <- read_netlogo_simul(file.path(outpath,"Amazon59years449PostRcp45_all2_distinct.csv"),skip=0)
...
# Load previous simulations and bind the total file
#
pexp0 <- readRDS(file.path("Simulations", "simExponentsAmazonRCP45_59postAll.rds")) 
saveRDS(bind_rows(pexp0,pexp), file.path("Simulations", "simExponentsAmazonRCP45_59postAll.rds"))
#saveRDS(pexp, file.path("Simulations", "simExponentsAmazonRCP45_59postAll.rds"))
...
save_plot("figure/Amazon_ModelVsData_SimulGAM.pdf",prow,base_width=8,base_height=6,dpi=600)
...
#
# Read Simulations based on RCP 4.5 and deforestation
#
pexp1 <- readRDS(file.path("Simulations", "simExponentsAmazonRCP45_59postDef.rds")) %>% filter(world_width==449) %>% 
                     mutate(tot_patch= tot_patch/(450*450),max_patch=max_patch/(450*450),,num_patch=num_patch/(450*450),
                    year=year(Date),decade=trunc((year - 2000 )/10)*10+2000,decade=paste0(decade , "-", decade+9),decade=factor(decade),theta=61,RCP="4.5 + Deforestation")
...
#
# Read Simulations based on RCP 8.5 and deforestation
#
pexp1 <- readRDS(file.path("Simulations", "simExponentsAmazonRCP85_59postDef.rds")) %>% filter(world_width==449) %>% 
                     mutate(tot_patch= tot_patch/(450*450),max_patch=max_patch/(450*450),,num_patch=num_patch/(450*450),
                    year=year(Date),decade=trunc((year - 2000 )/10)*10+2000,decade=paste0(decade , "-", decade+9),decade=factor(decade),theta=61,RCP="8.5 + Deforestation")
...
# Add Data
#
pexp <- pexp %>% bind_rows(patch_sizes_p %>% mutate(theta=61,RCP="4.5"), 
                           patch_sizes_p %>% mutate(theta=61,RCP="4.5 + Deforestation"),
                           patch_sizes_p %>% mutate(theta=61,RCP="8.5"),
                           patch_sizes_p %>% mutate(theta=61,RCP="8.5 + Deforestation")) %>%  mutate(tot_patch= 100 * tot_patch, max_patch= max_patch*100, num_patch=100* num_patch) %>% filter(year>2000)## Make %
pexp %>% count(decade,theta)
...
pdata <- pdata %>% bind_rows(patch_sizes_p %>% mutate(theta=61,RCP="4.5 + Deforestation") %>%  mutate(tot_patch= 100 * tot_patch, max_patch= max_patch*100, num_patch=100* num_patch)) %>% filter(year>2000)
pdata <- pdata %>% bind_rows(patch_sizes_p %>% mutate(theta=61,RCP="8.5 + Deforestation") %>%  mutate(tot_patch= 100 * tot_patch, max_patch= max_patch*100, num_patch=100* num_patch)) %>% filter(year>2000)

pexp <- pexp %>% mutate(fct=ifelse(RCP=="4.5","RCP 4.5",ifelse(RCP=="8.5","RCP 8.5",ifelse(RCP=="4.5 + Deforestation","RCP 4.5 + Deforestation","RCP 8.5 + Deforestation")))) 

pexp %>% group_by(fct,theta,year) %>% 
  filter( type == "pl", year>2000,decade!="Data") %>%  
  summarise(up_b=quantile(tot_patch,0.975),
            lo_b=quantile(tot_patch,0.025),
            tot_patch=median(tot_patch) ) %>%
  ggplot(aes(year,tot_patch,color=factor(theta),fill=factor(theta))) + 
  geom_line(size=2) + 
  ...
  theme(legend.position = c(0.8, 0.88)) + 
  geom_ribbon(aes(ymin=lo_b,ymax=up_b),alpha=0.2,linetype=0) + 
  facet_wrap(~fct,nrow=2) + 
  geom_point(data=pdata,color="black") +
  geom_vline(xintercept = 2021, linetype = "dashed", alpha = 0.5)


#ggsave("figure/Amazon_TotSize_year_theta61_RCP45-85Def.png",width=8,height=6,units="in",dpi=600)
ggsave("figure/Amazon_TotSize_year_theta61_RCP45-85Def.pdf",width=8,height=6,units="in",dpi=600)


# No Deforestation 4.5 8.5 old plot
#
p1 <- pexp  %>% group_by(random_seed,Date) %>% filter( type == "pl") %>% mutate(theta=factor(if_else(decade=="Data","Data",as.character(theta))), decade=if_else(decade=="Data", "2001-2021",decade)) %>% filter( !grepl("Deforestation",RCP))

p1 %>% ungroup() %>% mutate(decade=factor(decade),decade=fct_relevel(decade,c("2001-2021")), theta=fct_recode(theta, "Model"="61")) %>% ggplot(  aes(max_patch,tot_patch, color=theta ))+ geom_point(alpha=0.5) + theme_bw() +
      scale_color_viridis_d(name="")  + 
      xlab("Max Fire size (%)") +  
      ylab("Total Size (%)") +  
      theme(axis.text.x = element_text(angle = 90, hjust = 1))  + facet_grid( RCP ~ decade,drop=TRUE) +
#  theme(legend.position="bottom")
    theme(legend.justification=c(1,1), legend.position=c(.99,1))
ggsave("figure/Amazon_TotSizeVsMax_year_theta61_RCP45-85.png",width=8,height=6,units="in",dpi=600)

# Deforestation only 4.5
#
p1 <- pexp  %>% group_by(random_seed,Date) %>% filter( type == "pl") %>% mutate(theta=factor(if_else(decade=="Data","Data",as.character(theta))), decade=if_else(decade=="Data", "2001-2021",decade)) %>% filter(RCP == "4.5" | RCP == "4.5 + Deforestation")

p1 %>% ungroup() %>% mutate(decade=factor(decade),decade=fct_relevel(decade,c("2001-2021")), theta=fct_recode(theta, "Model"="61")) %>% ggplot(  aes(max_patch,tot_patch, color=theta, shape=theta ))+ geom_point(alpha=0.6) + theme_bw() +
      scale_color_viridis_d(name="")  + scale_shape(name="") +
      xlab("max fire size (%)") +  
      ylab("total size (%)") +  
      theme(axis.text.x = element_text(angle = 90, hjust = 1))  + facet_grid( RCP ~ decade,drop=TRUE) +
#  theme(legend.position="bottom")
    theme(legend.justification=c(1,1), legend.position=c(.99,1))
#ggsave("figure/Amazon_TotSizeVsMax_year_theta61_RCP45_Def.png",width=8,height=6,units="in",dpi=600)
ggsave("figure/Amazon_TotSizeVsMax_year_theta61_RCP45_Def.pdf",width=8,height=6,units="in",dpi=600)


# Deforestation only 8.5
#
p1 <- pexp  %>% group_by(random_seed,Date) %>% filter( type == "pl") %>% mutate(theta=factor(if_else(decade=="Data","Data",as.character(theta))), decade=if_else(decade=="Data", "2001-2021",decade)) %>% filter(RCP == "8.5" | RCP == "8.5 + Deforestation")
...
#  theme(legend.position="bottom")
    theme(legend.justification=c(1,1), legend.position=c(.99,1))
ggsave("figure/Amazon_TotSizeVsMax_year_theta61_RCP85_Def.png",width=8,height=6,units="in",dpi=600)



#
# Power Law exponent No Deforestation
#
pexp %>% group_by(random_seed,Date) %>% filter(AICc == min(AICc), type == "pl", !grepl("Deforestation",RCP)) %>% bind_rows(patch_dfp %>% mutate(theta=61,RCP="4.5")) %>% 
  bind_rows(patch_dfp %>% mutate(theta=61,RCP="8.5")) %>% mutate(decade=factor(decade),theta=factor(theta),decade=fct_relevel(decade,c("Data", "2000-2009","2010-2019","2020-2029")),theta=fct_recode(theta, "Model"="61")) %>% ggplot( aes(decade,expo,fill=decade)) + geom_violin() + scale_fill_viridis_d(guide=FALSE) +    scale_color_viridis_d() + ylab("Power Law Exponent")  +  stat_summary(fun.y=median, geom="point", size=2) + xlab("") + facet_grid( rows=vars(RCP) ,drop=TRUE,scales="free_x")+ theme(axis.text.x = element_text(angle = 50, hjust = 1))
ggsave("figure/Amazon_PatchExponent_theta61_RCP45-85.png",width=8,height=6,units="in",dpi=600)

# 
# Number of Fires No Deforestation
#
pexp %>% group_by(random_seed,Date) %>% filter( type == "pl", !grepl("Deforestation",RCP)) %>% mutate(decade=factor(decade),decade=fct_relevel(decade,c("Data","2001-2020"))) %>% ggplot(aes(decade,num_patch,fill=decade)) + geom_violin() + scale_fill_viridis_d(guide=FALSE) +    scale_color_viridis_d() + ylab("Number of Fires (%)")  +  stat_summary(fun=median, geom="point", size=2) + xlab("") + facet_grid(  rows=vars(RCP),drop=TRUE,scales="free_x")+ theme(axis.text.x = element_text(angle = 50, hjust = 1))
ggsave("figure/Amazon_Num_Fires_theta61_RCP45-85.png",width=8,height=6,units="in",dpi=600)

#
# Total size No Deforestation
#
pexp %>% group_by(random_seed,Date) %>% filter( type == "pl", !grepl("Deforestation",RCP)) %>% mutate(decade=factor(decade),decade=fct_relevel(decade,c("Data","2001-2020"))) %>% ggplot(aes(decade,tot_patch,fill=decade)) + geom_violin() + scale_fill_viridis_d(guide=FALSE) +    scale_color_viridis_d() + ylab("Total Fire Size (%)")  +  stat_summary(fun=median, geom="point", size=2) + xlab("") + facet_grid(  rows=vars(RCP),drop=TRUE,scales="free_x")+ theme(axis.text.x = element_text(angle = 50, hjust = 1))
ggsave("figure/Amazon_Tot_Size_theta61_RCP45-85.png",width=8,height=6,units="in",dpi=600)

# Max size No Deforestation
#
pexp %>% group_by(random_seed,Date) %>% filter( type == "pl", !grepl("Deforestation",RCP)) %>% mutate(decade=factor(decade),decade=fct_relevel(decade,c("Data","2001-2020"))) %>% ggplot(aes(decade,max_patch,fill=decade)) + geom_violin() + scale_fill_viridis_d(guide=FALSE)  +  ylab("Max Fire Size (%)")  +  stat_summary(fun.y=median, geom="point", size=2) + xlab("") + facet_grid(  rows=vars(RCP),drop=TRUE,scales="free_x") + theme(axis.text.x = element_text(angle = 50, hjust = 1))
ggsave("figure/Amazon_Max_Size_theta61_RCP45-85.png",width=8,height=6,units="in",dpi=600)

```

# Plots with no TerraClimate


# RCP 4.5 no terraclimate 

```{r readCompareSimRCP45-NoTerraClimate, echo=FALSE,message=FALSE,warning=FALSE,fig.align='center',eval=FALSE}
...
# Read Simulations based on RCP 4.5
#
pexp <- readRDS(file.path("Simulations", "simExponentsAmazonRCP45_59post.rds")) %>% filter(world_width==449) %>% 
                     mutate(tot_patch= tot_patch/(450*450),max_patch=max_patch/(450*450),,num_patch=num_patch/(450*450),
                    year=year(Date),decade=trunc((year - 2000 )/10)*10+2000,decade=paste0(decade , "-", decade+9),decade=factor(decade),theta=61,RCP="4.5")
...
#
# Read Simulations based on RCP 4.5 No Terraclimate
#
pexp1 <- readRDS(file.path("Simulations", "simExponentsAmazonRCP45_59postAll.rds")) %>% filter(world_width==449) %>% 
                     mutate(tot_patch= tot_patch/(450*450),max_patch=max_patch/(450*450),,num_patch=num_patch/(450*450),
                    year=year(Date),decade=trunc((year - 2000 )/10)*10+2000,decade=paste0(decade , "-", decade+9),decade=factor(decade),theta=61,RCP="4.5 No Terraclimate")
...
#
pexp <- pexp %>% bind_rows(patch_sizes_p %>% mutate(theta=61,RCP="4.5"), 
                           patch_sizes_p %>% mutate(theta=61,RCP="4.5 No Terraclimate")) %>%  
  mutate(tot_patch= 100 * tot_patch, max_patch= max_patch*100, num_patch=100* num_patch) %>% 
  filter(year>2000)## Make %
...
pexp <- pexp %>% mutate(fct=ifelse(RCP=="4.5","RCP 4.5",ifelse(RCP=="4.5 No Terraclimate","RCP 4.5 No Terraclimate",NA))) 


pexp %>% group_by(fct,theta,year) %>% 
  filter( type == "pl", year>2000,decade!="Data") %>%  filter(RCP %in% c("4.5","4.5 No Terraclimate")) %>%
  summarise(up_b=quantile(tot_patch,0.975),
            lo_b=quantile(tot_patch,0.025),
            tot_patch=median(tot_patch) ) %>%
  ggplot(aes(year,tot_patch,color=factor(theta),fill=factor(theta))) + 
...
ggsave("figure/Amazon_TotSize_year_theta61_RCP45-NoTerra.png",width=8,height=6,units="in",dpi=600)
