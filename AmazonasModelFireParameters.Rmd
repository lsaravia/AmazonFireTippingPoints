...
p1<- predict(gam03a,newdata=pr_test, se.fit = TRUE)
...
#
gdpp_exp <- gdpp %>% select(date,fit,ucl,lcl)
#
# repeat the last date
#
gdpp_exp <- gdpp_exp %>% add_row(date= gdpp_exp$date[nrow(gdpp_exp)] + months(1), fit= gdpp_exp$fit[nrow(gdpp_exp)])
write_csv(gdpp_exp %>% select(date,fit,ucl,lcl), "Data/Predicted_bF_rcp45.csv")
...
# add one row repeat the last month with 
...
#
gdpp_exp <- gdpp_exp %>% add_row(date= gdpp_exp$date[nrow(gdpp_exp)] + months(1), fit= gdpp_exp$fit[nrow(gdpp_exp)])
write_csv(gdpp_exp %>% select(date,fit,ucl,lcl), "Data/Predicted_bF_rcp85.csv")


```


# Calcular max y min anual para ver la tendencia!!!!!!!!!!!!