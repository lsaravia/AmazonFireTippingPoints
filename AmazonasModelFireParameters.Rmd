...
#
# Model with seasonal term and ciclic 
#
...
pr_test %>% ggplot(aes(date,bF)) + geom_point() +theme_bw() + scale_colour_viridis_d() + theme(axis.text.x = element_text(angle = 45, hjust = 1,size=7),) +  geom_line(aes( x= date, y= fit )) +  ylab("𝑓") + 
  geom_ribbon(aes(x=date,ymin=lcl,ymax=ucl),alpha=0.3) 
ggsave("figure/Amazon_bF_prediction.jpg",width=8,height=6,units="in",dpi=600)
...
summary(gam03a)
gam.check(gam03a)
taic <- tibble::rownames_to_column(AIC(gam01,gam01a,gam02,gam03,gam03a,gam03b))  %>% mutate(deltaAIC=AIC - min(AIC)) %>% arrange(deltaAIC)
knitr::kable(taic)
#
# Check gamma model
#
...
pr_train <- subset(pr, year(date)<2015 | year(date)>2017 )
pr_test  <- subset(pr, year(date)>=2015 & year(date)<=2017 ) 
pr_period = "2015-2018"


pr_train <- subset(pr, year(date)<2010 | year(date)>2012 )
pr_test  <- subset(pr, year(date)>=2010 & year(date)<=2012 ) 
pr_period = "2010-2013"
...
pr_test %>% ggplot(aes(date,bF)) + geom_point() +theme_bw() + scale_colour_viridis_d() + theme(axis.text.x = element_text(angle = 45, hjust = 1,size=7)) + geom_line(aes( x= date, y= fit )) +  ylab("𝑓") + 
  geom_ribbon(aes(x=date,ymin=lcl,ymax=ucl),alpha=0.3) 
ggsave("figure/Amazon_bF_prediction2018-2021.jpg",width=8,height=6,units="in",dpi=600)
...
# Second best
#
gam03at <- gam(log(bF) ~ te(tmmx,pr1, bs =c("tp","tp"), k =c(12,12) ), data=pr_train,method="REML",
             family = gaussian)
...
# 3rd best
#
gam03t <- gam(log(bF) ~ te(tmmx,pr, bs =c("tp","tp"), k =c(12,12) ), data=pr_train,method="REML",
             family = gaussian)
p1<- predict(gam03t,newdata=pr_test, se.fit = TRUE)
pr_test <- pr_test %>% mutate(fit=p1$fit,se.fit =p1$se.fit, ucl=exp(fit + (1.96 * se.fit)), lcl = exp(fit - (1.96 * se.fit)), fit=exp(fit)  )
pr_test %>% ggplot(aes(date,bF)) + geom_point() +theme_bw() + scale_colour_viridis_d() + theme(axis.text.x = element_text(angle = 45, hjust = 1,size=7)) + geom_line(aes( x= date, y= fit )) +  
  geom_ribbon(aes(x=date,ymin=lcl,ymax=ucl),alpha=0.3) 

pr_test %>% summarise( MAPE=mean(abs((bF - fit)/bF))*100)
model_mape <- bind_rows(model_mape,pr_test %>% summarise( MAPE=mean(abs((bF - fit)/bF))*100) %>% mutate(model="gam03t",period=pr_period))
...
model_mape <- bind_rows(model_mape,pr_test %>% summarise( MAPE=mean(abs((bF - fit)/bF))*100) %>% mutate(model="gam04bt",period=pr_period))
...
|     MAPE|model   |period    |
|--------:|:-------|:---------|
| 54.47824|gam03bt |2018-2021 |
| 56.77618|gam03at |2018-2021 |
| 56.95208|gam03t  |2018-2021 |
| 63.78738|gam04bt |2018-2021 |
| 28.52796|gam03bt |2015-2018 |
| 37.46660|gam03at |2015-2018 |
| 48.12553|gam03t  |2015-2018 |
| 30.95800|gam04bt |2015-2018 |
| 50.52010|gam03bt |2010-2013 |
| 70.47736|gam03at |2010-2013 |
| 75.13256|gam03t  |2010-2013 |
| 58.87268|gam04bt |2010-2013 |
...
pr_exp <- bind_rows(pr %>% mutate(date = fecha) ,pr) %>% dplyr::select(date,bF)
...
# Add gamma distribution to model extreme values
#
#    shape 
# 1.421544 
#    rate 
...
# Add only to the months 8,9,10
#
# gdpp <- gdpp %>% mutate(deltaGamma = ifelse(month %in% c(8,9,10), rgamma(nrow(pr_test),1.421544, rate=13736.42),0), newFit = fit + deltaGamma)
# 
# # New plots with extreme values 
# #
# gdpp %>% ggplot(aes(date,bF)) + geom_point() +theme_bw() + scale_colour_viridis_d() + theme(axis.text.x = element_text(angle = 45, hjust = 1,size=7)) + geom_line(aes( x= date, y= newFit )) +  
#     geom_ribbon(aes(x=date,ymin=lcl,ymax=ucl),alpha=0.3) + ggtitle("RCP 4.5") 
# ggsave("figure/Amazon_bF_RCP4.5PlusGamma.png",width=8,height=6,units="in",dpi=600)
