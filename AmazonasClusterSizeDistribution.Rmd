...
if(file.exists(patch_size_name)) {
  patch_size <- readRDS(patch_size_name) %>% mutate(date=ymd(date))%>% filter(date>="2001-01-01" & date<"2021-01-01" )
} else {
...
# patch_dfp <- patch_dfp %>% group_by(region,date,period) %>% slice_tail(n=4)
...
#
knitr::kable(patch_dfp %>% group_by(date) %>% filter(AICc == min(AICc)) %>% select(date,type:range,exp_p_value))
#
...
p1 <- patch_sizes_p %>% filter(date>2000, date<2021) %>% group_by(date) %>% 
  summarize(max_patch=max(size)/total_forest*100,tot_patch=sum(size)/total_forest*100) %>% 
  mutate( label =as.character(date)) %>%
   ggplot(  aes(max_patch,tot_patch, label=label,color=date ))+ geom_point() + theme_bw() +
      scale_color_viridis_c(guide=NULL)  + 
...
## power Law range and median 

  `range(expo)` `median(expo)`
          <dbl>          <dbl>
1          2.00           2.29
2          2.46           2.29
