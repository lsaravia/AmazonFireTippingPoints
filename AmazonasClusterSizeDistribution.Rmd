...
#
knitr::kable(patch_dfp %>% group_by(date) %>% filter(AICc == min(AICc)) %>% dplyr::select(date,type:range,exp_p_value))
#
...
p1 <- patch_sizes_p %>% filter(date>2000, date<2022) %>% group_by(date) %>% 
  summarize(max_patch=max(size)/total_forest*100,tot_patch=sum(size)/total_forest*100) %>% 
  mutate( label =as.character(date)) %>% inner_join(hansen_def, by=c("date"="year"))
...
cor(p1[,c("percent_deforested","tot_patch")])
